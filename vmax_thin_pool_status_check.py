...
def get_srp_pools(symmid):
    ...
    pools = list()
    xml_string = run_command('symcfg -sid '+symmid+' list -pool -thin -output xml_e')
...
##############################################################################
def find_tdev_pool_alloc(symmid):
    log.debug("symmmid: %s", symmid)
    # Find all VMAX pools
    ...
    #for elem in tdevtree.iter('Symmetrix/ThinDevs/Device'):
    for elem in tdevtree.iter('DevicePool'):
        tdev_name = get_xml_text(elem, 'pool_name')
        log.debug("LUN: %s", tdev_name)
        tdevs_dict[tdev_name]['total_tracks_gb'] = get_xml_text(elem, 'total_tracks_gb')
        tdevs_dict[tdev_name]['alloc_tracks_gb'] = get_xml_text(elem, 'total_used_tracks_gb')
        tdevs_dict[tdev_name]['free_tracks_gb'] = get_xml_text(elem, 'total_free_tracks_gb')
        tdevs_dict[tdev_name]['alloc_percent'] = get_xml_text(elem, 'percent_full')
...
def print_tdevs(pools, tdevs):
    header = list()
    header.append("TDEV")
    header.append("Total tracks GB")
    header.append("Alloc tracks GB")
    header.append("Free tracks GB")
    header.append("Alloc Percent (%)")
    ...
    for tdev in sorted(tdevs):
        ...
        row.append(tdevs[tdev]['free_tracks_gb'])
...
def main():
    ...
    pools, tdevs_dict = find_tdev_pool_alloc(args.sid)
